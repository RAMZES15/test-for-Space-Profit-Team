...
function YsNo() {
  ...
  $("#yeas").click(() => {
    ...
    process = true;
    scrollDown();
    addToForm('#input-allergy','allergic')
  });
  $("#no").click(() => {
    $(".chat-content-buttons-gender").css("display", "none");
    myMassange("Нет");
    process = true;
    scrollDown();
    addToForm('#input-allergy','not allergic')
  });
}
...
//my code
function addToForm(className, answer){
...
function validateName() {
  ...
  const nameHint = $('#input-name-hint');

  if (name.length < 2 || /\d/.test(name)) {
    nameHint.text("Ім'я повинно містити щонайменше 2 символи і не містити цифри");
  } else {
    nameHint.text("");
  }
}
...
function validatePhone() {
  ...
  const phoneHint = $('#input-phone-hint');

  if (!/^(\+?380|380)\d{9}$/.test(phone)) {
    phoneHint.text("Номер телефону повинен починатися з +380 або 380 і містити 9 цифр не рахуючи 380");
  } else {
    phoneHint.text("");
  }
}
...
function validateForm() {
  ...
  if (name.length < 2) {
    nameHint.text("Ім'я повинно містити щонайменше 2 символи");
    return false;
  }

  if (/\d/.test(name)) {
    nameHint.text("Ім'я не може містити цифри");
    return false;
  }
...
$('.submit-roulette').click(function(event) {
  if (!validateForm()) {
...
$(document).ready(function() {
  ...
  $('.glow-button').click(function() {
    if (visibleComments < totalComments) {
      let nextVisible = visibleComments + 3;
      $('.comment-item:lt(' + nextVisible + ')').addClass('comment-item-show');
      visibleComments = nextVisible;
...
function scrollToBottom() {
  ...
  container.animate({ scrollTop: scrollHeight }, animationSpeed);
}
...
$('.header__hamburger').add('.overlay').click(function() {
